...
const EquipmentSports = ({ equipment }) => {
  const { _id, image, itemName, price, processingTime, stockStatus, brand } =
    equipment;
      ...
      <div className="flex bg-base-100 shadow-xl flex-1 flex-col rounded-xl p-1 gap-1">
        <figure className="relative">
          <img
            src={image}
            alt={itemName}
            className="h-52 w-72 mx-auto rounded-2xl"
          />

          <p
            className={`${
              stockStatus > 0 ? " bg-green-500" : "bg-red-500"
            } p-2 rounded-xl text-white text-xl absolute top-0 right-0`}
          >
            {stockStatus > 0 ? " In Stock" : "Out Of Stock"}
          </p>
        </figure>
        <div className="flex flex-col font-barlow rounded-b-2xl bg-base-100 p-3 m-2 gap-2">
          ...
          <p className="text-xl ">Delivery Time : {processingTime} </p>
